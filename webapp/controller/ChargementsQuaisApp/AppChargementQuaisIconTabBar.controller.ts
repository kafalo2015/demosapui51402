...
export default class AppChargementQuaisIconTabBar extends Controller {
  ...
  //===================================LOT 18 END -> Simplification du routing (une seule route et une seule target pour l'ensemble des quais)===============================================//
    ...
    oEventBus?.subscribe("Default","chargementQuaiButtonEvent",this._onChargementQuaiButton,this); // GEMINI[Amélioration] Un module EventRegistrationService a été crée pour les handler
  //----------------------------------------------------------------------------------------------------------------------------//
  //             HANDLER validationDialogEvent  [LOT 8 : Validation des messages de chargement]  
  //----------------------------------------------------------------------------------------------------------------------------//
   
  // GEMINI [TO dELETE) [Ancien code de subscribe avec fonctions fléchées]
  // this.getOwnerComponent()?.getEventBus().subscribe("Default",application_events_enum.validation_dialog_event,(channel:string,event:string,data: Object) => {           
    ...
    //    const current_quai_index_json = aQuaisData.findIndex(obj => obj.quai === (payload.quai));   //Object.values(data)[0] contient le libellé du quai (QUAI08, QUAI09,ect)
    ...
    //   {    
      ...
      // setTimeout(() => {
        ...
        // Dans onInit
this.getOwnerComponent()?.getEventBus().subscribe("Default",application_events_enum.validation_dialog_event,this._onValidationDialogEvent, this );

  }

// Nouvelle méthode privée
private _onValidationDialogEvent(channel: string, event: string, data: object): void {
    const payload = data as IChargementMsgValidation;
    const oComponent = this.getOwnerComponent() as Component;
    const oModelData = oComponent.getModel("chargementModelJson") as JSONModel;
    const aQuaisData = oModelData.getProperty("/results") as any[];
    
    const current_quai_index_json = aQuaisData.findIndex(obj => obj.quai === payload.quai);

    if (payload.quai_number === this.gv_current_quai_number) {
        this.onOpenDialogValidCharg().then(() => {
            this.gv_dialog_validation_charg.setBindingContext(
                oComponent.getModel("validationMsgChargementQuaiModelJSON")?.createBindingContext(
                    `/results/${current_quai_index_json}/`
                ) as Context,
                "validationMsgChargementQuaiModelJSON"
            );
...
private _triggerQuaiTransition(oWrapper: VBox, iNewQuaiNumber: number, iDuration: number): void {
  ...
  //------------------------------------------------------------------------------------------------------------------------------------------------------------------------ //
    public button_chargementquai_handler() : void{ 
      ...
      router.navTo("RouteChargementQuai", {quainumber: this.gv_current_quai_number});    // Evolution quai 8 et 09s
      // LOT 18 BEGIN Simplification routing [Uilisation de navTo au lieu  target display]
          }
...
// 2. Faire le binding directement ici (Lot 18/19)
  ...
  if (t_validation_msg_list.length > 0 ) {  
       this.open_validationMsg_popup();
  }
}

private open_validationMsg_popup()
{
  console.log("P1 LOT18  Appel de la boîte de dialogue de validation") ;
  const payload : IChargementMsgValidation =  { quai: this.gv_current_quai, quai_number:   this.gv_current_quai_number};
...
//----------------------------------------------------------------------------------------------------------------------------//
  ...
  // Dans ton onObjectMatched ou _performContextBinding

// GEMINI BEGIN [Résolution Bug] On s'assure que le modèle de chargement des quais a eu le temps de se charger avant de rechercher l'indice d'un quai donné dans le modèle et de binder sur un quai donnné
oModelData.dataLoaded().then(() => {
  const aQuaisData = oModelData.getProperty("/results") as any[];
  if (aQuaisData) {
...
// GEMINI END [Résolution Bug] On s'assure que le modèle de chargement des quais a eu le temps de se charger avant de rechercher l'indice d'un quai donné dans le modèle et de binder sur un quai donnné
}
...
router.navTo("RouteChargementStart", {quainumber: quai_number}); 
  ...
  // GEMINI [Capitalisation] Add dependante pour connecter le dialogue à une vue donnée, dans ce cas pas besoin de faire setModel IMPORTANT : Connecte le dialogue à la vue pour l'i18n et les modèles
 ...
 public onConfirmValidationMsgChargement(event:Button$PressEvent):void {
  //--------------  MODELE  Récupération du modèle de notifications des quais (Messages de validation par quai)--------------------------------------------//
 ...
 //--------------  CONTEXTE Contexte d'appel de la validation----------------------------------------------------------------------------------------------//
...
public onExit(): void {
  ...
  if (oEventBus) {
        // Désabonnement groupé
        oEventBus.unsubscribe("Default", "chargementQuaiButtonEvent", this._onChargementQuaiButton, this);
        oEventBus.unsubscribe("Default", application_events_enum.validation_dialog_event, this._onValidationDialogEvent, this);
        
        console.log("Lot 21 : Nettoyage complet des handlers EventBus effectué.");
    }

    // Destruction physique du dialogue pour libérer la RAM
    if (this.gv_dialog_validation_charg) {
