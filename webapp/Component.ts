        ...
        export default class Component extends BaseComponent {
            ...
            public init() : void {
                ...
                this._gv_environment = environment_enum.prod;
    ...
    // Récupération des données du modèle
        ...
        // Appel de l'api de fin de chargement    
            finChargementQuaiModelJSON.loadData(this._apiService.gv_endchargement_api_url , JSON.stringify(input_data.tMotifNocharg)    ,true,  "POST", false, true, mHeader)?.then(result=>{
            const router = this.getRouter();

            // BEGIN LOT 18/LOT 19 -> Vue quai unique et simplification du routing
            // =>Appeler la route chargement start du quai en cours
            this.getEventBus().publish("Default", application_events_enum.chargement_end_event, {});      //=> TODO -> Voir s'il est toujours nécessaire d'appeler cet évènement
            const lv_quai_number : string =  input_data.quai1?.slice(4,6);
            console.log("Navigation vers le formulaire de démarrage de chargement du quai :" + lv_quai_number );
            router.navTo("RouteChargementStart", {quainumber: lv_quai_number });
        ...
        //----------------------------------------------------------------------------------------
                ChargementStartModel.loadData(this._apiService.gv_startchargement_api_url,"",true,  "POST", false, true, mHeader)?.then(result=>{
                //let lv_target_quai : string;     //LOT 18/19-- 
                MessageToast.show("Chargement démarré sur le quai : " + i_quai ,{ duration: 3000, width : '50%' })
                //========================= BEGIN LOT 18/LOT 19 -> Vue quai unique et simplification du routing/ A TESTER =====================================================//
            ...
            //      router.getTargets()?.display(lv_target_quai);           // TODO -> Remis après refonte du modèle de notification car manquant
                ...
                const lv_quai_number : string =  i_quai?.slice(4,6);
                console.log("Navigation vers le formulaire de démarrage de chargement du quai :" + lv_quai_number );
